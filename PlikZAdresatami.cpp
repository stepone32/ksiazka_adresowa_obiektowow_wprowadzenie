...
void PlikZAdresatami::edytujWybranegoAdresataWPliku(Adresat edytowanyAdresat)
{
    ...
    liniaZDanymiAdresataOddzielonePionowymiKreskami = zamienDaneAdresataNaLinieZDanymiOddzielonymiPionowymiKreskami(edytowanyAdresat);
    ...
    if (odczytywanyPlikTekstowy.good() == true)
    {
        while (getline(odczytywanyPlikTekstowy, wczytanaLinia))
        {
            ...
            if(edytowanyAdresat.pobierzIdAdresata()== idWczytanegoAdresata)
            {
...
void PlikZAdresatami::usunWybranegoAdresataZPliku(int idUsuwanegoAdresata)
{
    ...
    if (odczytywanyPlikTekstowy.good() == true && idUsuwanegoAdresata != 0)
    {
        ...
        {
            idWczytanegoAdresata=pobierzIdAdresataZDanychOddzielonychPionowymiKreskami(daneJednegoAdresataOddzielonePionowymiKreskami);

             // Tych przypadkow jest tyle, gdyz chcemy osiagnac taki efekt,
            // aby na koncu pliku nie bylo pustej linii
            if (idWczytanegoAdresata == idUsuwanegoAdresata) {numerUsuwanejLinii=numerLiniiWPlikuTekstowym;}
...
int PlikZAdresatami::podajIdOstatniegoAdresataPoUsunieciuWybranegoAdresata(int idUsuwanegoAdresata, int idOstatniegoAdresata)
{
    ...
    {
        return pobierzIdOstatniegoAdresataZPliku();
    }
    else
    {
        return idOstatniegoAdresata;
